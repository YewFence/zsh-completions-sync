...
该命令会运行 `zcs generate project` 以生成项目级别的补全脚本到当前目录的 `.completions/zsh`，并在进入目录时自动加载。

### 单个工具补全脚本生成

`zcs` 支持针对单个工具生成补全脚本，命令格式是 `zcs generate tool` 或 `zcs generate project tool`，其中 `tool` 是工具名或工具别名。比如：

```zsh
zcs generate pnpm
zcs generate project node
```
...
ZCS_OUTPUT_DIR=.completions/custom zcs generate project
```
